 ...
 *
 *   #nonogram/W/H/DATA?name=Cat&answer=Cat%20in%20a%20hat
 *
...
export function buildShareLink(base: string, p: Puzzle, opts: { includeAnswer?: boolean } = {}): string {
  ...
  return `${base}#nonogram/${p.width}/${p.height}/${encodePuzzlinkData(p)}${q}`;
}
...
export function puzzleFromLocation(loc: { hash: string; search: string }): LocationPuzzle {
  ...
  try {
    ...
    const source = hash.startsWith("nonogram/") ? hash : search.startsWith("nonogram/") ? search : null;
    if (source) {
